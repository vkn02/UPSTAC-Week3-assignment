...
public class ConsultationController {
  ...
  public List<TestRequest> getForConsultations() {
    log.info("Retrieving the list of pending consultations ...");
  ...
  public TestRequest assignForConsultation(@PathVariable Long id) {
    try {
      // Get the logged in doctor user and assign the consultation.
      User doctor = userLoggedInService.getLoggedInUser();
      log.info(
          "Assigning the consultation request with id- "
              + id
              + " to Doctor- "
              + doctor.getUserName());
  ...
  public TestRequest updateConsultation(
    ...
    try {
      // Get the logged in doctor user and update the consultation.
      User doctor = userLoggedInService.getLoggedInUser();
      log.info(
          "Updating the consultation request with id- "
              + id
              + " by Doctor- "
              + doctor.getUserName());

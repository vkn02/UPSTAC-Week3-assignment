...
public class UserService {
        ...
        .ifPresent(
            user1 -> {
              throw new AppException(
                  "User with Same Phone number already exists " + user.getPhoneNumber());
            });
  }
  ...
  public User addUserWithRole(
      @Valid RegisterRequest registerRequest, Role role, AccountStatus status) {
  ...
  public User updateApprovalStatus(Long userId, AccountStatus status) {
    User user =
        userRepository.findById(userId).orElseThrow(() -> new AppException("Invalid User ID"));

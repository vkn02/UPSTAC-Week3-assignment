...
public class AppInitializationService implements ApplicationListener<ApplicationReadyEvent> {
  ...
  private static Gender getRandomGender() {
    ...
    if (gender.equals(Gender.FEMALE) && canSetOtherGender()) return Gender.OTHER;
    else return gender;
  }
  ...
  static String getRandomDoorNumber() {
    ...
    String myChar =
        getRandomItemInStrings(Arrays.asList("", "A", "B", "C", "D", "E", "F", "G", "H"));

    if (myChar.equalsIgnoreCase("")) return "" + res;
    else return res + "/" + myChar;
  }
  ...
  static String getRandomAddress(Integer pinCode) {
    ...
    if (addresses.containsKey(pinCode)) return addresses.get(pinCode);
    else return getRandomDoorNumber() + " - " + getRandomStreetName() + ",Goa";
  }
  ...
  public void addDefaultUserData() {
    ...
    govtAuthority =
        userService.addGovernmentAuthority(
            createRegisterRequestWith("authority", getRandomPinCode()));
  }
